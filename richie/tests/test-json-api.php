...
class Test_JSON_API extends WP_UnitTestCase {
    ...
    public function test_get_assets_list_with_combined_custom() {
        update_option( 'richie_assets', json_decode('[{"local_name": "test/test2/script.js", "remote_url": "http://example.org/test/test2/script.js"}]') );
        ...
        $this->assertEquals( $last->local_name, RICHIE_APP_ASSET_URL_PREFIX . 'test/test2/script.js' );
    ...
    public function test_get_assets_list_with_combined_and_overriding_custom() {
        update_option( 'richie_assets', json_decode('[{"local_name": "wp-includes/js/jquery/jquery.js", "remote_url": "http://another.org/wp-includes/js/jquery/jquery2.js?ver=1.12.4-wp"}]') );
        $request  = new WP_REST_Request( 'GET', '/richie/v1/assets' );
        $response = $this->server->dispatch( $request );
        $this->assertEquals( 200, $response->get_status() );
        ...
        $this->assertEquals( $first->local_name, RICHIE_APP_ASSET_URL_PREFIX . 'wp-includes/js/jquery/jquery.js' );
