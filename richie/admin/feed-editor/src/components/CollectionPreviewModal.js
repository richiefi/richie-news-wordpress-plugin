...
const PLACEHOLDER_IMAGE =
  'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300"%3E%3Crect fill="%23ddd" width="400" height="300"/%3E%3Ctext fill="%23999" font-family="sans-serif" font-size="48" dy="10.5" font-weight="bold" x="50%25" y="50%25" text-anchor="middle"%3E%3F%3C/text%3E%3C/svg%3E';
...
export default function CollectionPreviewModal({ collectionId, onClose }) {
  ...
  const renderArticle = (article, index, headerTitle = '') => {
    ...
    const bgColor = article.background_color
    ...
    if (layout === 'featured') {
      return (
        <div key={index} className={articleClassName} style={articleStyle}>
          {headerTitle && (
            <h3 className="preview-header-title">{headerTitle}</h3>
          )}
  ...
  return (
    ...
    >
      <div className="collection-preview-notice">
        <p>
          {__(
            'Preview shows the feed structure and layout types. Actual appearance depends on the native app configuration.',
            'richie'
          )}
        </p>
      </div>
      ...
      <div className="modal-footer collection-preview-footer">
