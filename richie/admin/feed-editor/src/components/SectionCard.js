...
export default function SectionCard( { section, onEdit, onDelete } ) {
  ...
  const backgroundColor = section.background_color ? `#${ section.background_color }` : null;
  ...
  const handleDelete = () => {
    if ( window.confirm( __( 'Are you sure you want to delete this section?', 'richie' ) ) ) {
      onDelete();
    }
  };
  ...
  if ( ! headerTitle && preview && preview.articles && preview.articles.length > 0 ) {
    const headerArticle = preview.articles.find( ( article ) => article.collection_header_title );
  ...
  const configuredCountLabel = showPreviewCountNote
    ...
    : sprintf( _n( '%d article', '%d articles', configuredCount, 'richie' ), configuredCount );

  return (
    <div
      ref={ setNodeRef }
      style={ cardStyle }
      className={ `feed-item-card section-card${ isDragging ? ' is-dragging' : '' }${
      ...
      <div className="card-content">
          ...
          <div className="card-header-details">
            <div className="card-title-row">
              <div className="card-title-group">
                ...
                <span className="card-meta">{ configuredCountLabel }</span>
              </div>
        ...
        <div className="card-preview">
          { isLoadingPreview ? (
            ...
            <>
              { headerTitle && <h4 className="preview-header-title">{ headerTitle }</h4> }
              <ul className={ `preview-list${ isExpanded ? ' preview-list--expanded' : '' }` }>
                { ( isExpanded ? preview.articles : preview.articles.slice( 0, 3 ) ).map(
                  ( article, index ) => (
                    <li key={ index } className="preview-item">
                      { article.thumbnail && (
                        <img src={ article.thumbnail } alt="" className="preview-thumbnail" />
                      ) }
            ...
            <span className="preview-empty">{ __( 'No articles found', 'richie' ) }</span>
          ) }
        </div>
      </div>

      <div className="card-actions">
        <Button icon={ edit } label={ __( 'Edit', 'richie' ) } onClick={ onEdit } isSmall />
        <Button
          icon={ trash }
