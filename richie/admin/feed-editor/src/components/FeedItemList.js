...
export default function FeedItemList( {
  ...
  const activeItem = activeId ? items.find( ( item ) => item.uniqueId === activeId ) : null;
      ...
      <DragOverlay>
        { activeItem ? (
          activeItem.type === 'source' ? (
            <SectionCard section={ activeItem } onEdit={ () => {} } onDelete={ () => {} } />
          ) : (
            <AdSlotCard adSlot={ activeItem } onEdit={ () => {} } onDelete={ () => {} } />
          )
        ) : null }
      </DragOverlay>
    </DndContext>
